...
export function SocialShareDialog({ open, onClose, post }: SocialShareDialogProps) {
  ...
  function composeUrl(platform: string, caption: string): string | null {
    switch (platform) {
      ...
      case "telegram":
        return `https://t.me/share/url?url=${encodeURIComponent(postUrl)}&text=${encodeURIComponent(caption)}`
      case "whatsapp":
        return `https://wa.me/?text=${encodeURIComponent(caption)}`
      case "reddit":
        return `https://reddit.com/submit?url=${encodeURIComponent(postUrl)}&title=${encodeURIComponent(post.title.slice(0, 300))}`
      case "pinterest":
        return image
          ? `https://pinterest.com/pin/create/button/?url=${encodeURIComponent(postUrl)}&media=${encodeURIComponent(image)}&description=${encodeURIComponent(caption)}`
          : `https://pinterest.com/pin/create/button/?url=${encodeURIComponent(postUrl)}&description=${encodeURIComponent(caption)}`
      default:
        return null
    }
  }
  ...
  const handleAction = async (platform: PlatformConfig) => {
    ...
    const url = composeUrl(platform.id, caption)

    if (platform.action === "api") {
      setPosting(platform.id)
      try {
        const res = await fetch("/api/admin/social/post", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ platform: platform.id, postId: post.id }),
        })
          ...
          {platforms.map((platform) => {
            ...
            return (
              <button
                ...
                disabled={isPosting}
                className="w-full text-left rounded-xl border-2 border-gray-200 dark:border-[#374151] hover:border-[#F59E0B] hover:bg-amber-50 dark:hover:bg-amber-900/20 transition-all bg-white dark:bg-transparent overflow-hidden disabled:opacity-60"
              >
                {/* Platform header */}
                ...
                <div className="px-4 pb-3">
                  {isPosting ? (
                    <span className="inline-flex items-center justify-center w-full text-xs font-medium px-3 py-1.5 rounded-lg text-white"
                    ...
                    <span className="inline-flex items-center justify-center w-full text-xs font-medium px-3 py-1.5 rounded-lg bg-green-600 text-white">
                      <Check className="h-3 w-3 mr-1.5" /> Posted successfully
                    </span>
                  ) : platform.action === "api" ? (
                    <span className="inline-flex items-center justify-center w-full text-xs font-medium px-3 py-1.5 rounded-lg text-white"
                      style={{ backgroundColor: platform.color }}
                  ...
                  ) : platform.action === "clipboard" ? (
                    <span className="inline-flex items-center justify-center w-full text-xs font-medium px-3 py-1.5 rounded-lg text-white"
                      style={{ backgroundColor: platform.color }}
                    >
                      Copy & Open <ExternalLink className="h-3 w-3 ml-1.5" />
                    </span>
                  ) : (
                    <span className="inline-flex items-center justify-center w-full text-xs font-medium px-3 py-1.5 rounded-lg text-white"
                      style={{ backgroundColor: platform.color }}
                    >
                      Open compose <ExternalLink className="h-3 w-3 ml-1.5" />
                    </span>
                  )}
                </div>
              </button>
