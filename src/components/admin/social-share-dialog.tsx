...
interface Platform {
  ...
  getUrl: (caption: string, title: string, url: string, excerpt: string, image: string) => string | null
  canPreFill: boolean
}
...
const platforms: Platform[] = [
  {
    ...
    getUrl: (caption: string, _t: string, url: string, _e: string, _i: string) =>
      `https://www.facebook.com/dialog/share?app_id=${FB_APP_ID}&display=popup&href=${encodeURIComponent(url)}&quote=${encodeURIComponent(caption)}&redirect_uri=${encodeURIComponent(url)}`,
  },
  {
    id: "twitter",
    name: "X / Twitter",
    ...
    getUrl: (caption: string, _t: string, _u: string, _e: string, _i: string) =>
  ...
  },
  {
    ...
    getUrl: (_c: string, _t: string, url: string, _e: string, _i: string) =>
  ...
  },
  {
    ...
    getUrl: (caption: string, _t: string, url: string, _e: string, _i: string) =>
      `https://reddit.com/submit?url=${encodeURIComponent(url)}&title=${encodeURIComponent(caption)}`,
  },
  {
    id: "pinterest",
    name: "Pinterest",
    ...
    getUrl: (caption: string, _t: string, url: string, _e: string, image: string) =>
      image
        ? `https://pinterest.com/pin/create/button/?url=${encodeURIComponent(url)}&media=${encodeURIComponent(image)}&description=${encodeURIComponent(caption)}`
...
export function SocialShareDialog({ open, onClose, post }: SocialShareDialogProps) {
  ...
  const openPlatform = async (platform: Platform) => {
    const url = platform.getUrl(caption, post.title, postUrl, excerpt, image)

    if (url && platform.canPreFill) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={onClose}>
      ...
      >
        ...
        <div className="px-5 py-3 bg-gray-50 dark:bg-[#1F2937] border-b border-gray-200 dark:border-[#374151] shrink-0">
        ...
        <div className="overflow-y-auto px-5 py-4 flex-1 space-y-3">
          <p className="text-xs font-medium text-gray-500 dark:text-[#6B7280]">
            Click a platform to post
          </p>

          {platforms.map((platform) => {
            const hasUrl = platform.getUrl("x", "x", "", "", "") !== null
            const isCopied = copied === platform.id
            const previewLines = caption.split("\n").filter(Boolean)

            return (
              <button
                key={platform.id}
                onClick={() => openPlatform(platform)}
                className="w-full text-left rounded-xl border-2 border-gray-200 dark:border-[#374151] hover:border-[#F59E0B] hover:bg-amber-50 dark:hover:bg-amber-900/20 transition-all bg-white dark:bg-transparent overflow-hidden"
              >
                <div className="flex items-center gap-3 px-3.5 pt-3 pb-2">
                  ...
                  <p className="text-sm font-semibold text-gray-900 dark:text-[#F9FAFB]">{platform.name}</p>
                  {isCopied && (
                ...
                <div className="px-3.5 pb-2">
                  <div className="bg-gray-50 dark:bg-[#0A0F1E] rounded-lg px-3 py-2.5 border border-gray-100 dark:border-[#1F2937]">
                    <p className="text-xs text-gray-700 dark:text-gray-300 leading-relaxed line-clamp-3">
                      {previewLines[0] && (
                        <span className="font-semibold">{previewLines[0]}</span>
                      )}
                      {previewLines.length > 1 && (
                        <>
                          ...
                          {previewLines.slice(1).map((line, i) => (
                            <span key={i}>
                              {line}
                              {i < previewLines.slice(1).length - 1 && <br />}
                            </span>
                          ))}
                        </>
                      )}
                ...
                <div className="px-3.5 pb-3">
                  <span className="inline-flex items-center justify-center w-full text-xs font-medium px-3 py-1.5 rounded-lg transition-colors text-white"
                    style={{ backgroundColor: platform.color }}
                  >
                    {hasUrl ? (
                      <>Post to {platform.name} <ExternalLink className="h-3 w-3 ml-1.5" /></>
                    ) : (
                      <>Copy caption <Check className="h-3 w-3 ml-1.5" /></>
                    )}
                  </span>
                </div>
              </button>
