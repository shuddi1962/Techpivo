...
function AIAssistantTab({ articles, categories, stats }: AIAssistantTabProps) {
  ...
  const QUICK_ACTIONS = [
    { label: "What to write today?", icon: "🎯", prompt: "Based on current search demand data, what 5 specific tech topics should TechPivo publish today? Give exact article titles, the search volume for each keyword, and why that topic is the right call right now. Focus on actionable recommendations, not theory." },
    { label: "Content gaps to fill", icon: "🔍", prompt: "Analyze our portfolio and identify 5 specific content gaps — high-search-volume tech keywords that competitors rank for but we don't cover. For each gap, give the exact article title and the search volume/CPC/competition that makes it worth covering." },
    { label: "How-to keywords", icon: "📚", prompt: "List 10 high-traffic 'how to' or 'what is' keywords in tech, programming, AI, cybersecurity, or gadgets that we should target. For each, include search volume, CPC, competition level, and a recommended article title that would rank quickly." },
    { label: "Review opportunities", icon: "⭐", prompt: "Based on trending tech products and gadgets in 2026, recommend 5 specific review topics with exact headlines. For each, include the target keyword, search volume, and why a comprehensive review would rank well in the next 30 days." },
    { label: "Tutorials needed", icon: "🛠️", prompt: "Recommend 5 step-by-step tutorial topics with high search demand in programming, AI, or developer tools. For each, give the exact headline, the audience pain point, the search volume, and why TechPivo is uniquely positioned to rank for that topic." },
    { label: "AI news angles", icon: "🤖", prompt: "What's happening in the AI space right now (Gemini, ChatGPT, Claude, open-source LLMs, agents, automation) that we should cover urgently? Give 5 specific story angles with exact headlines, the news hook, and the official sources to cite." },
    { label: "Comparison articles", icon: "⚖️", prompt: "List 5 high-traffic 'X vs Y' comparison pairs in tech, AI, or gadgets. For each, give the exact search query people use, the search volume, and the article structure that would rank quickly (features, pricing, verdict)." },
    { label: "Seasonal topics now", icon: "📅", prompt: "What tech topics are people searching for more than usual right now in 2026? List 5 specific article ideas tied to current events, product launches, or industry moments. Include the target keyword and search volume for each." },
  ]
