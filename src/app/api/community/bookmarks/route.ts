...
export async function POST(request: NextRequest) {
  ...
  const rl = checkRateLimit(`bookmark:${clientIp(request)}`, RATE_LIMITS.bookmark);
  if (!rl.allowed) {
    return NextResponse.json({ error: 'Too many bookmark actions. Try again later.' }, { status: 429 });
  }
