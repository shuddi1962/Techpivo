...
- 2026-08-07: **Agent Reach bugfix (938f339, deployed 9pk0tosr3):** All channels broke because the page sent `{q}`/`{url}` but the route read `query`/`url`. Search, YouTube, GitHub, LinkedIn, and suggestions ALL returned 400s ("Query required"/"URL required") — web + rss were the only working ones. Fixed: route now normalizes `const text = String(query || q || url || "")` for every channel. All 8 channels verified live via tsx (search 10 results, web 1, youtube 1, github 10, rss 20, linkedin 1, suggest 7, trending 24).
- 2026-08-07: User token VERIFIED (vcp_84RA... — whoami OK, account goodnewsonyematara2020-8483, project access OK). Stored in `.env.vercel` (gitignored, NOT committed). PREVIOUS token (vcp_5JOw...) was invalid (404 User not found) — replaced. NOTE: never write the token value into ADMIN_SESSION.md — GitHub push protection blocks commits containing Vercel token patterns. ALSO: the token works for `vercel whoami`/`ls` but NOT for `vercel deploy` ("token not valid") — use the saved CLI login for deploys: `npx vercel --prod --yes --non-interactive` (no --token).
