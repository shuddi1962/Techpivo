...
- **WEB IMAGES = DEFAULT SITE-WIDE + AI-WRITE 400 FIX + SOURCE BLOCKS & FAKE IMAGES REMOVED FROM LIVE DB (8d0f9aa, PUSHED) — editor Web tab first (default), all server image flows web-first (Google CSE → keyless Bing → Wikimedia) with Pexels fallback, Pexels hardcoded keys removed, 5 posts + 117 posts + 5 keyword_articles cleaned live via Management API (Source/Sources sections + injected `<figure><img loading="lazy">` blocks stripped, 0 remaining), AI-write no longer 400s (responseMimeType removed), article page revalidate 60s + hardened tags, SafeImage referrerPolicy no-referrer, AI write auto-fetches web featured image** — User: "now they are three Pexels / Web / Library use the web own for the general image purpose site wide". Verified BEFORE push: tsc EXIT=0 (fixed 1 error: postTags `t` implicit any → typed), lint 0 errors (pre-existing img/exhaustive-deps warnings only), vitest 53/53, dev smoke 6/6 (/links 200, live article `/vintage-reborn-microsoft-word-11a-gets-native-x64-port-for-modern-windows-msliwkav` 200, /api/pexels 200 fallback intact, /api/google-images?query=sony a7r vi&engine=auto 200 → **source=bing items=20 real images** (mixvale.com.br Sony A7), POST /api/preview-drafts 401, POST /api/admin/ai-write 401). **AI-write 400 ROOT CAUSE FIXED**: Gemini forbids `responseMimeType: 'application/json'` in generationConfig when tools are attached — removed from `src/lib/ai-rewriter.ts` (validate() already parses JSON from plain text via extractJsonObject/repairJson chain from 35891e1). **NEW `src/lib/web-images.ts`** — single shared lib: `StockImageItem {src, alt, link?, license?}`, `searchGoogle` (GOOGLE_API_KEY+GOOGLE_CX safe=active), `searchBing` (keyless live scrape, murl blocks), `searchWikimedia` (keyless), `searchWebImages(query, engine)` (auto = google→bing→wikimedia chain), `searchWebImage`, `searchFeaturedImage` (**web-first + Pexels fallback** so server flows never end up imageless; PEXELS_API_KEY env only). `/api/google-images` → thin wrapper (same params/response, no exports). **Server flows all switched to searchFeaturedImage**: breaking-news-rewrite, research-keyword, keywords/generate, content-health/fix, fetch-images + rewrite-all (their searchPexels wrappers delegate; hardcoded Pexels API key REMOVED from both). **Source/Sources attribution blocks GONE from code**: breaking-news-rewrite (URL branch `<section><h2>Source</h2><p>Based on <a>the original story</a>…rewritten and expanded by TechPivo…` + topic branch `<section><h2>Sources</h2><p>This article was researched using Google Search via Gemini 2.5 Flash…`) + research-keyword externalLinks + keywords/generate Sources append — all deleted; Related Resources (internal links) KEPT. **Live DB cleanup via Management API (xkhvojjogoeuvrifekwr)**: counts before = 5 posts + 5 keyword_articles with Source sections, 117 posts with injected figures; ran `regexp_replace(content, '<section>\s*<h2>Source[s]?</h2>.*?</section>', '', 'gis')` WHERE content ~ 'rewritten and expanded by TechPivo|researched using Google Search via Gemini' + `regexp_replace(content, '<figure>\s*<img[^>]*loading="lazy"[^>]*>\s*</figure>', '', 'gis')` on posts + keyword_articles (content col verified) — re-verified: **0/0/0/0 remaining**, 238 posts intact. **SEO pipeline**: seo/fix `fixContentImages` now STRIPS injected figures + resolves issue (injectImagesIntoContent deleted, stripInjectedImages added, pickImages kept for featured fix); seo/audit no longer emits `no_content_images` (imgCount still used by mediaScore). **Editor**: featured-image-panel + rich-text-editor — Web tab FIRST + `webSource` default `"web"` (was pexels). **AI write success now auto-fetches featured image**: if `!post.featured_image`, panel calls `/api/google-images?query=<first 5 words of headline>&engine=auto`, sets featured_image + og_image + twitter_image, shows note "Featured image auto-fetched from live web search (bing)". **Article page**: `revalidate` 3600 → **60** (edits appear within a minute), tags hardened (Array.isArray / string split / else []), quickBrief already coerces; SafeImage gains `referrerPolicy="no-referrer"` (bypasses hotlink protection for web images). next.config.mjs verified: catch-all `{ hostname: '**' }` ALREADY at line 53 → arbitrary web hosts optimize fine, no change needed. Working tree CLEAN after push.**
...
| Date | Action | Summary |
|------|--------|---------|
| 2026-08-15 | **WEB IMAGES = DEFAULT SITE-WIDE + AI-WRITE 400 FIX + SOURCE BLOCKS & FAKE IMAGES REMOVED FROM LIVE DB (8d0f9aa, PUSHED)** | User: "now they are three Pexels / Web / Library use the web own for the general image purpose site wide". Verified BEFORE push: tsc EXIT=0 (fixed 1 error: postTags `t` implicit any → typed), lint 0 errors (pre-existing img/exhaustive-deps warnings only), vitest 53/53, dev smoke 6/6 (/links 200, live article `/vintage-reborn-microsoft-word-11a-gets-native-x64-port-for-modern-windows-msliwkav` 200, /api/pexels 200 fallback intact, /api/google-images?query=sony a7r vi&engine=auto 200 → **source=bing items=20 real images** (mixvale.com.br Sony A7), POST /api/preview-drafts 401, POST /api/admin/ai-write 401). **AI-write 400 ROOT CAUSE FIXED**: Gemini forbids `responseMimeType: 'application/json'` in generationConfig when tools are attached — removed from `src/lib/ai-rewriter.ts` (validate() already parses JSON from plain text via extractJsonObject/repairJson chain from 35891e1). **NEW `src/lib/web-images.ts`** — single shared lib: `StockImageItem {src, alt, link?, license?}`, `searchGoogle` (GOOGLE_API_KEY+GOOGLE_CX safe=active), `searchBing` (keyless live scrape, murl blocks), `searchWikimedia` (keyless), `searchWebImages(query, engine)` (auto = google→bing→wikimedia chain), `searchWebImage`, `searchFeaturedImage` (**web-first + Pexels fallback** so server flows never end up imageless; PEXELS_API_KEY env only). `/api/google-images` → thin wrapper (same params/response, no exports). **Server flows all switched to searchFeaturedImage**: breaking-news-rewrite, research-keyword, keywords/generate, content-health/fix, fetch-images + rewrite-all (their searchPexels wrappers delegate; hardcoded Pexels API key REMOVED from both). **Source/Sources attribution blocks GONE from code**: breaking-news-rewrite (URL branch `<section><h2>Source</h2><p>Based on <a>the original story</a>…rewritten and expanded by TechPivo…` + topic branch `<section><h2>Sources</h2><p>This article was researched using Google Search via Gemini 2.5 Flash…`) + research-keyword externalLinks + keywords/generate Sources append — all deleted; Related Resources (internal links) KEPT. **Live DB cleanup via Management API (xkhvojjogoeuvrifekwr)**: counts before = 5 posts + 5 keyword_articles with Source sections, 117 posts with injected figures; ran `regexp_replace(content, '<section>\s*<h2>Source[s]?</h2>.*?</section>', '', 'gis')` WHERE content ~ 'rewritten and expanded by TechPivo|researched using Google Search via Gemini' + `regexp_replace(content, '<figure>\s*<img[^>]*loading="lazy"[^>]*>\s*</figure>', '', 'gis')` on posts + keyword_articles (content col verified) — re-verified: **0/0/0/0 remaining**, 238 posts intact. **SEO pipeline**: seo/fix `fixContentImages` now STRIPS injected figures + resolves issue (injectImagesIntoContent deleted, stripInjectedImages added, pickImages kept for featured fix); seo/audit no longer emits `no_content_images` (imgCount still used by mediaScore). **Editor**: featured-image-panel + rich-text-editor — Web tab FIRST + `webSource` default `"web"` (was pexels). **AI write success now auto-fetches featured image**: if `!post.featured_image`, panel calls `/api/google-images?query=<first 5 words of headline>&engine=auto`, sets featured_image + og_image + twitter_image, shows note "Featured image auto-fetched from live web search (bing)". **Article page**: `revalidate` 3600 → **60** (edits appear within a minute), tags hardened (Array.isArray / string split / else []), quickBrief already coerces; SafeImage gains `referrerPolicy="no-referrer"` (bypasses hotlink protection for web images). next.config.mjs verified: catch-all `{ hostname: '**' }` ALREADY at line 53 → arbitrary web hosts optimize fine, no change needed. Working tree CLEAN after push. |
