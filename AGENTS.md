...
- **GLOBAL FULL-WIDTH LAYOUT FIX + PAGE EDITOR HERO IMAGE SEARCH (346584a + 54e912a, PUSHED) — root-cause architectural fix for site-wide layout not spanning full viewport width; page editor hero image now has web search (Web/Pexels tabs matching post editor)** — User: "The whole layout is now broken on my end, what happened? The layout is no longer full width, there is also blank space on the right of every page" + "the hero image upload section of the page editor should also have the same web image search option we have in the post editor page". **LAYOUT ROOT CAUSE (5-file fix)**: `.site-main` had `max-width: 1400px; margin: 0 auto;` (centered + constrained); `.main-layout` (CSS grid) had NO max-width; redundant `container` class inside `.main-layout` caused double-constraint (1400px + 1280px). FIX (1) `globals.css` — removed max-width/margin from `.site-main` (now just `padding: 24px`), added `max-width: 1400px; margin: 0 auto;` to `.main-layout`, added `.full-bleed` utility + responsive padding; (2) `page.tsx` (homepage) — HeroSection + AdSlot + SiteBlock moved OUTSIDE `.site-main`; (3) `category/[slug]/page.tsx` — container removed, header given full-bleed; (4) `category/[slug]/[subcategory]/page.tsx` — same; (5) `[slug]/page.tsx` (article) — container removed, featured image full-bleed, card restructured. Verified BEFORE push: tsc EXIT=0, lint 0 errors. **HERO IMAGE SEARCH (`src/app/admin/pages/[slug]/page.tsx`, +111 lines)**: Added 6 state vars (`heroSearchQuery/Results/searching/SearchError/SearchSource/ActiveSource`), `searchHeroImages()` handler calling `/api/google-images?engine=auto` + `/api/pexels`, Web/Pexels tab UI with search input (Enter key), results grid with selection highlighting + license badges + source indicator. tsc EXIT=0. Carry-over user items unchanged: delete Vercel dashboard cron `fetch-trending-keywords` 03:45 UTC, real AdSense publisher ID (placeholder pub-3867128949973557 + enable_auto_ads=false).
...
| Date | Action | Summary |
|------|--------|---------|
| 2026-08-25 | **GLOBAL FULL-WIDTH LAYOUT FIX + PAGE EDITOR HERO IMAGE SEARCH (PUSHED 346584a + 54e912a)** | User: "The whole layout is now broken on my end, what happened? The layout is no longer full width, there is also blank space on the right of every page" + "the hero image upload section of the page editor should also have the same web image search option we have in the post editor page". **LAYOUT ROOT CAUSE (5-file fix)**: `.site-main` had `max-width: 1400px; margin: 0 auto;` (centered + constrained); `.main-layout` (CSS grid) had NO max-width; redundant `container` class inside `.main-layout` caused double-constraint (1400px + 1280px). FIX (1) `globals.css` — removed max-width/margin from `.site-main` (now just `padding: 24px`), added `max-width: 1400px; margin: 0 auto;` to `.main-layout`, added `.full-bleed` utility + responsive padding; (2) `page.tsx` (homepage) — HeroSection + AdSlot + SiteBlock moved OUTSIDE `.site-main`; (3) `category/[slug]/page.tsx` — container removed, header given full-bleed; (4) `category/[slug]/[subcategory]/page.tsx` — same; (5) `[slug]/page.tsx` (article) — container removed, featured image full-bleed, card restructured. Verified BEFORE push: tsc EXIT=0, lint 0 errors. **HERO IMAGE SEARCH (`src/app/admin/pages/[slug]/page.tsx`, +111 lines)**: Added 6 state vars (`heroSearchQuery/Results/searching/SearchError/SearchSource/ActiveSource`), `searchHeroImages()` handler calling `/api/google-images?engine=auto` + `/api/pexels`, Web/Pexels tab UI with search input (Enter key), results grid with selection highlighting + license badges + source indicator. tsc EXIT=0. Carry-over user items unchanged: delete Vercel dashboard cron `fetch-trending-keywords` 03:45 UTC, real AdSense publisher ID (placeholder pub-3867128949973557 + enable_auto_ads=false). |
