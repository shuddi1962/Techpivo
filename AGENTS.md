...
- **Current Phase:** **Analytics Center + Report Center rebuild LIVE (commits d89399c, 79fc570, 5cbf2b8, a8654dc); migrations 046-047 NOT yet applied to live DB — user must paste SQL in Supabase SQL Editor** — Analytics page: Overview splits **Site Views vs Post Views** (dual-line 30d chart + separate KPIs), **Sessions = distinct session_id** (new column, tracked per-tab), Real-Time shows Active Visitors + Sessions Today, Audience tab real data (device/browser/os — old tab queried NONEXISTENT columns), Newsletter tab fixed (`subscribed_at` + real `newsletter_sends`), Exports real CSV w/ session/device columns + Daily Views CSV. **Competitors tab REMOVED. Revenue tab = realtime transaction center (commit 79fc570)**: affiliate revenue gone; live subscription on `ad_revenue` + `ad_campaign_daily_stats` + `ad_campaigns` (unique channel `analytics_revenue_*`, removeChannel, 30s poll + focus refresh); KPIs: Ad Revenue (30d $), Campaign Spend (₦, live/paused/completed), Impressions, Clicks, CTR, Pending Orders; 30d ComposedChart revenue bars + impressions line; Revenue by Source list; Recent Transactions (headline/email/₦total_price/status badge w/ colors); deleted unused `revenue-analytics.tsx`. **Report Center rebuilt**: `src/lib/reports.ts` (MD/CSV builders for all 6 reports from REAL data), `/api/admin/reports` (generate + schedule CRUD, admin/editor guard, service-role), `/api/cron/reports` (CRON_SECRET, sendBrandedEmail), page with generate MD/CSV/Export All + Schedule Reports UI (→ `report_schedules`). Quick stats fixed (`profiles` + `subscribers`). **Scheduled reports via Supabase pg_cron (migration 047, commit 5cbf2b8)**: `report_cron_dispatcher()` SECURITY DEFINER function reads Bearer secret from `site_settings.cron_secret` (random uuid generated AT APPLY TIME — never committed) and `net.http_get`s https://techpivo.com/api/cron/reports hourly (`cron.schedule 'techpivo-report-cron'`, idempotent unschedule, pg_cron + pg_net extensions, EXECUTE revoked from PUBLIC/anon/authenticated, postgres only). **Cron auth centralized (commit a8654dc)**: new `src/lib/cron-auth.ts` — `getCronSecret()` = CRON_SECRET env OR `site_settings.cron_secret` (DB wins only when env unset); `isCronAuthorized(req, {required})`; ALL 9 cron-guarded routes now use it (strict: fetch-rss, write-keyword-article, fetch-trending-keywords, indexnow, ingest, cron/reports; optional: social-posts, google-indexing, submit-indexing — open-if-unconfigured preserved, auto-protected once 047 seeds DB secret). ingest's indexnow call uses `getCronSecret()`. No Vercel env/cron needed. **Migrations 046 + 047** (NOT applied): 046 = +session_id/device/browser/os on analytics_events + index, `report_schedules` (RLS + realtime), +`ad_campaign_daily_stats` to realtime publication; 047 = pg_cron dispatcher + cron_secret seed. Trackers send session_id/device/browser/os via `src/lib/view-tracking.ts`; `/api/increment-views` stores them. **IMPORTANT: features depending on 046-047 (distinct Sessions, Audience device/browser/os, report scheduling + delivery, live daily-stats, DB-secret cron auth) stay dead until user runs both migrations — CLI/Management API 403 (stored Supabase token = wrong account; no DB password in .env.local), so it CANNOT be applied from code.**
