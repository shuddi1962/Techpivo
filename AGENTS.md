...
- **PAGES CENTER ENHANCEMENT COMMITTED (0f6f396) — rich page editor + placement controls + visibility + delete + shop + corrections-policy + editorial-policy** — 9 files, +307/−28. NEW `src/app/editorial-policy/page.tsx` + `src/app/corrections-policy/page.tsx` (PageShell pattern). `src/lib/pages.ts`: NEW `DesignSettings` interface (hero_bg/text_color/content_width/hero_alignment/hero_height) + marketplace entry in SITE_PAGES. Migration 082 (`082_pages_placement_design_settings.sql`): `placement` (text, default 'both', CHECK header/footer/both/none) + `design_settings` (JSONB) on `site_pages` + index. API route: delete action + placement/design_settings handling on create/update. Admin editor: placement selector (4 buttons), design settings panel, delete button with confirm. Header/Footer: respect `placement` field + `isPublic`/`isInHeader`/`isInFooter` from realtime `use-site-pages`. DB seeded: editorial-policy (footer), corrections-policy (footer), marketplace (both + hero_image). Verified BEFORE push: tsc EXIT=0 (NODE_OPTIONS=--max-old-space-size=4096), next lint 0 errors, vitest 12 files / **133 tests** EXIT=0. Carry-over user item unchanged: real AdSense publisher ID (placeholder pub-3867128949973557 + enable_auto_ads=false).
