...
- **Tools round 2: bugfixes + live is_active + full 55-tool audit (commit pending)** — (1) **pdf-lib "Invalid color" fixed**: pdf-lib v4 requires `rgb()`/`grayscale()` helper colors — plain `{r,g,b}` objects throw at draw time (excel-to-pdf now uses `rgb()`). (2) **PDF read failures fixed**: pdfjs worker now served from `/public/pdf.worker.min.mjs` (copied from `pdfjs-dist/legacy/build/`; `?url` import removed) — bundler-proof in prod; also `doc.destroy()` w/ cleanup + `console.error` diagnostics in pdf-to-excel. (3) **Admin activate/deactivate now bulletproof**: new guarded `POST /api/admin/tools` (requireAdminRole + service-role write, slug/is_active validation) + optimistic UI update — no more silent RLS failures. (4) **is_active now reflects on public pages**: new `src/components/tools/tool-status.tsx` — `ToolStatusGate` (detail page shows "unavailable" panel; public RLS only returns active rows so absence = inactive) + `ActiveToolGroup` (hub hides inactive cards, client fetch of active slugs deduped via module-level promise). FIXED RSC 500: you CANNOT pass render functions from server→client components — props must be serializable (data only). (5) `/api/geo` never returns 400 anymore — graceful "unknown" payload (client falls back to NGN). **Full audit of all 55 tools**: registry↔metadata↔TOOL_LIST 100% consistent (55/55, no dups, all have faq+keywords); markdown preview safe (ALL content escaped before HTML injection); no eval/Function anywhere (JSON/JWT use JSON.parse); `/api/tools/net` validated (hostname charset + length + record-type whitelist + 8s timeout); image tools capped (8192px/80MP guard); regex tester text capped at 20k chars (ReDoS guard); CSV export now **formula-injection safe** (cells starting =,+,@,-letter,tab,CR get `'` prefix — toCsv in tools-dev.tsx).**Smoke tests (dev server, all PASSED)**: all 55 `/tools/[slug]` pages 200 + `/tools` 200 + gate SSR rendered; `/api/tools/fx` live source 200 (USD→NGN 1363.19), `/api/tools/net` MX lookup 200, `/pdf.worker.min.mjs` 200. NOTE: interactive button clicks can't be tested headlessly (no browser here) — logic verified by code review; runtime errors now surface with real messages via console.error. tsc + next lint clean.
