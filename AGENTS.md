...
- **AI-WRITE GOOGLE TOOL FIX + IN-ARTICLE WEB IMAGES + ARTICLE PAGE IMAGE FIX (a0f3a36, PUSHED) — googleSearchRetrieval removed (topic + URL modes share one tools array), AI write now returns featured image + inserts real web figures into article body (one per H2 section), article page featured image container height fixed (was 0-height → image invisible)** — User: "please fix this the in the post editor page the ai writer from topic to not work it shows this AI writing failed. Debug: http_400: { error: { code: 400, message: 'google_search_retrieval is not supported. Please use google_search tool instead.', status: 'INVA..." + "remove this from url in that section aswell" + "if am using ai to write once i put a topic/keyword it should handle everything but the full article, tags, images, noth featured, and inarticle either ananlytics, chart, diagram, infrographics from the web image that discribe the article and any other thing to handle it" + "for the public page the image in each sign article page does not show, it only shows the tirle of the article not as you saw it before you cliked the aritcle". Verified BEFORE push: tsc EXIT=0, lint 0 errors (pre-existing warnings only), vitest 53/53, dev smoke 6/6 (article page `/vintage-reborn-microsoft-word-11a-gets-native-x64-port-for-modern-windows-msliwkav` 200 + SSR contains `h-72` container + `<img>`, /api/google-images 200 source=bing items=20, /api/pexels 200, POST /api/preview-drafts 401, POST /api/admin/ai-write 401). **1. Gemini tool 400 ROOT CAUSE**: `src/lib/ai-rewriter.ts` sent `tools: [{googleSearch:{}}, {googleSearchRetrieval:{dynamicRetrievalConfig:{...}}}]` — gemini-2.5-flash REJECTS `google_search_retrieval` ("google_search_retrieval is not supported. Please use google_search tool instead.") → every manual AI write (topic mode AND url mode — both use the same tools array) failed with http_400. FIX: removed `googleSearchRetrieval`, tools = `[{ googleSearch: {} }]` only (line ~494). **2. AI write handles EVERYTHING now**: NEW `enrichArticleWithWebImages(content, topic, maxFigures=2)` in `src/lib/web-images.ts` — searches a web featured image (web-first + Pexels fallback via searchFeaturedImage) AND inserts REAL relevant web figures into the article body: extracts H2 headings (regex `/<h2[^>]*>(.*?)<\/h2>/gi` — NO `s` flag, tsconfig target is es2017), queries up to 4 section titles via `searchWebImage` (parallel Promise.all), inserts `<figure><img src alt loading="lazy" /></figure>` after each matched heading's following `</p>` (or right after `</h2>`), up to maxFigures inserts; alt text HTML-escaped (escapeAttr), heading text stripped of tags + 80-char cap. Wired into: **`/api/admin/ai-write`** (editor AI panel, both modes) — route now enriches `result.article.content`, sets `featured_image`/`og_image`/`twitter_image` on the returned article (spread `{...result.article, ...}` — AIArticle type has NO image fields, so fresh fields, no TS error); **`/api/admin/breaking-news-rewrite`** + **`/api/admin/research-keyword`** — replaced their `searchFeaturedImage(searchTerm)` first call with `enrichArticleWithWebImages(article.content, searchTerm, 2)` → `enriched.content` becomes finalContent base (Related Resources still appended after), featured falls back to `enriched.featuredImage || searchFeaturedImage(category fallback)`. **Panel**: `ai-writing-panel.tsx` `featured = (a as {featured_image?: string}).featured_image || post.featured_image || ""` (server-supplied featured now wins; the client-side `/api/google-images` fallback fetch stays only when server found nothing). **3. Public article page image invisible ROOT CAUSE**: `src/app/[slug]/page.tsx` featured card container was `relative w-full bg-muted max-h-72 md:max-h-96 overflow-hidden` — NO explicit height, and SafeImage with `fill` renders `relative w-full h-full` wrapper + absolutely-positioned Image → parent auto-height = 0 → image collapsed to 0×0 (page showed only the title; cards on listings have real heights so they showed fine). FIX: `h-72 md:h-96` (explicit heights, overflow-hidden kept); `sizes` prop NOT added (SafeImageProps has no sizes — TS2322). Working tree CLEAN after push.**
