...
- **BUILD FIX COMMITTED (6930b4a, PUSHED) — Vercel build now GREEN end-to-end** — User's Vercel log showed commit 7b5226b failing with `Type error: Route "src/app/api/community/posts/route.ts" does not match the required types of a Next.js Route. "titleSimilarity" is not a valid Route export field.` (Next.js App Router Route contract: only GET/POST/PUT/PATCH/DELETE/HEAD/OPTIONS allowed; ANY other named export — even a test-imported helper — fails the type-check). Fix: moved `titleTokens` + `titleSimilarity` into NEW `src/lib/title-similarity.ts`; route imports it; test moved to `src/lib/__tests__/title-similarity.test.ts`. **Local `next build` additionally exposed a fatal prerender error on `/community/create`: `useSearchParams() should be wrapped in a suspense boundary`** — it is a 'use client' page using `useSearchParams()` with no Suspense → hard BUILD_EXIT=1 (had been silently broken since P3-P5 commit 2a6026c). Fixed with the same pattern as search/unsubscribe: inner `CreatePage` component keeps the hook, default export wraps in `<Suspense fallback=...>`. Verified: local `next build` (NODE_OPTIONS=--max-old-space-size=4096) EXIT=0, 272/272 pages generated, `/community/create` now `○` static; 53/53 tests green; pushed → Vercel auto-deploys via GitHub integration (log showed `Cloning github.com/shuddi1962/Techpivo`, no token needed). NOTE: PageShell pages (/about, /contact, /advertise, /privacy-policy, etc.) log caught `DYNAMIC_SERVER_USAGE` "PageShell fetch failed" errors during prerender — they render as `ƒ` dynamic + fall back to registry defaults at build; non-fatal, intentional (live DB row fetched per-request at runtime).
