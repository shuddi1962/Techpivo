...
| Date | Action | Summary |
|------|--------|---------|
| 2026-08-16 | **OPENROUTER FALLBACK + PARAGRAPH WEB IMAGES + AUTO-FIX THRESHOLDS (commit pending) — manual AI research falls back to OpenRouter when Gemini's daily manual cap/429/validate fails; in-article web figures now also fill paragraph gaps when articles lack usable H2s; post-editor auto-fix checks (keyword_in_slug via slugify, word-boundary keyword density) actually reach their checklist thresholds with NEW paragraph_length fix; per-post draft keys; red LIVE sidebar badges** | User: "What did we do so far?" mid-round (continued the in-flight round from `f7cde3f`). Verified BEFORE push: tsc EXIT=0 (fixed ONE TS2345 — type-predicate body in web-images spare filter needs explicit `s !== null && s !== undefined` — `Boolean(s)` narrowing unavailable), lint 0 errors (pre-existing post-editor-provider exhaustive-deps only), vitest 77/77 (7 files). **1. Auto-fix checks now pass for real**: (a) `seo-utils.ts` — `keyword_in_slug` check was `state.slug.includes(keyword)` (plain include, mismatch with dashed slugs) → now `countInText(slugify(keyword), state.slug)`; NEW exported `countKeywordOccurrences(text, keyword)` word-boundary regex `(^|[^a-z0-9])${escaped}([^a-z0-9]|$)` "gi" — used by `keywordDensity` AND `ensureKeywordDensity` (editor-autofix) so density counts are consistent. (b) `splitSentenceIntoTwo` (editor-autofix) HARD fallback when no bestEnd/fallbackEnd found: walk words to charEnd near `target = max(6, floor(maxWords*0.85))`, strip trailing `[\s,;]+$`, return `[first + ".", second.charAt(0).toUpperCase() + second.slice(1)]` — marathon 60-word sentences now split regardless of clause structure. (c) NEW `splitLongParagraphs(html, maxWords=150)` + `findParagraphSplitPoints(inner, maxWords, minChunk)` — `\.\s+` scan skipping inside tags (lastClose < lastOpen) and URLs (`https?:\/\/|www\.` in preceding 80 chars), both chunks ≥ `max(40, maxWords*0.6)`; rebuilds via `"</p><p>"` at reversed points. Wired into seo-panel: NEW fixable `paragraph_length` Fix item (`updatePost({content: splitLongParagraphs(post.content)})`) + Fix All chain (keywordSlug → keywordTitle → insertKeywordSentence → keywordFirstHeading → ensureKeywordDensity → improveReadability → splitLongParagraphs → addInternalLinks). **2. Draft keys per post (post-editor-provider)**: `draftKey(id?)` = `techpivo-editor-draft-<id>` | `techpivo-editor-draft-new`; LEGACY single-key `techpivo-editor-draft` migrated once when no post id; `clearDraftKeys(id?)` removes BOTH; restore strips `id`/`status`/`published_at`/`scheduled_at` (editing a published post never mixes with a NEW-post draft again — save-failure backup also writes the per-post key); saveDraft/publish/schedule success all clear keys. **3. In-article web images paragraph fallback (web-images.ts)**: `enrichArticleWithWebImages` — when `inserted < maxFigures`, remaining unused web results are inserted after first/middle/last `<p>` close-tag positions (points from pRe scan; picks [0] / [0,1] / [0,mid,last]; reversed insertion keeps positions valid; alt `${r.alt || cleanTopic} — ${cleanTopic}`). All 3 manual routes (ai-write, research-keyword:98, breaking-news-rewrite:132) call with 3 — now works even for articles without H2 headings. **4. OpenRouter fallback for manual writes (ai-rewriter.ts)**: NEW `manualWriteWithOpenRouter(prompt)` — key site_settings `openrouter_api_key` → env OPENROUTER_API_KEY; model site_settings `openrouter_model` → env OPENROUTER_MODEL → default `google/gemini-2.5-flash`; POST `https://openrouter.ai/api/v1/chat/completions` (Content-Type/Bearer/`HTTP-Referer: SITE_URL` — now imported from @/lib/constants/X-Title, messages system "ONLY valid raw JSON" + user prompt, max_tokens 8192, temp 0.45, AbortSignal.timeout(90000), NO response_format to avoid 400s); parse via `validate(raw, "openrouter")` (validate sets modelUsed from its param); debug strings `openrouter_no_key` / `openrouter_http_<status>:<err>` / `openrouter_empty:<err>` / `openrouter_validate:<reason>` / `openrouter_error:<msg>`. Wired into `manualWriteFromTopic` + `manualWriteFromUrl`: Gemini fails → warn → fallback → first success wins; total failure returns combined `debug: "<gemini> / <openrouter>"`. `AIArticle.modelUsed` widened `'gemini-grounded' | 'openrouter'` (all consumers write it to text col `model_used` — safe). ai-write route: usage log model now `article.modelUsed === "openrouter" ? "openrouter-fallback" : "gemini-2.5-flash"`; research-keyword 500 copy "AI research failed across all providers"; agent-reach retry copy (page.tsx:382 "retries across multiple providers automatically") verified already accurate. **5. sidebar.tsx LIVE badges red**: `link.badge === "LIVE" ? "bg-red-500 text-white" : "bg-muted text-muted-foreground"` + font-semibold (Breaking News, Agent Reach). **6. OpenRouter route cross-checked**: existing `/api/openrouter` route uses same site_settings key/model pattern (its default model mixtral-8x7b-instruct untouched; no response_format there either).** |
