...
- **Current Phase:** **Ad Marketplace — Self-Serve Auction Model live (migrations 044 + 045 applied live; commit pending)** — Google/Meta Ads style: advertisers set their own **daily budget + bid (CPM or CPC)** against placement **min_bid floors** (`min_bid_cpm`/`min_bid_cpc` on ad_placements, ₦500-1000 CPM / ₦50-100 CPC by tier), 10 currencies with live fx_rates in site_settings, campaign goals + CTA types + target audience (country/device/interest chips), video ads on video-capable placements, AI creative generator (Gemini), public /advertise page in desktop + mobile nav, **advertiser account area** `/account/ads` (My Ads list + `/account/ads/[id]` detail with KPI cards + 14-day chart from `ad_campaign_daily_stats` + owner Pause/Resume/Delete + `/account/ads/new` create flow). NOTE: 044 was pivoted from fixed weekly/monthly pricing to auction bidding after being drafted — apply the FINAL 044 (incl. min_bid columns, widened billing_model CHECK 'cpm'/'cpc', daily-stats table + RLS, fx_rates seed) + 045 (RPC ambiguity fix) before V2 UI works.
