...
- **AI WRITE JSON PARSE DEFENSE-IN-DEPTH (PUSHED 9c1f4b7) — `validate:json_parse_fail_after_object_extract` recurrence (user: "Gemini research failed. Manual quota OK (1/50). Gemini debug: validate:json_parse_fail_after_object_extract" in agent research + breaking news) root-caused: `escapeInnerQuotes()` from d7d1b1a closed a string on ANY quote followed by `, } ] :` — but `,` and `:` also appear in prose (`"the "AI", model is great"`, `"the "term": is used"`), so a legit closing quote got escaped, leaving the real close quote to terminate the string early → `"AI chips boom"`-style corruption → JSON.parse "Bad control character"/"Unexpected string". Fix (src/lib/ai-rewriter.ts): (1) close-heuristic rework — after a quote, skip JSON whitespace (`' \t\r\n'.includes()` — Gemini often pretty-prints, so `,` + `\n` + `"` defeated the old space-only skip) then: `} ]`/EOF → close; `,` → close ONLY if the next token is `"`/`{`/`[` (a new value start); `:` → close ONLY if the next token is a JSON value start (`"`/`{`/`[`/digit/`-`/t/f/n); else emit `\"`. Open-condition (prev char `{ , [ :`) is pre-existing and unchanged. Accepted gaps (documented, untested): prose `"term": 2026` and prose containing `"}"`. Test case `'{"content":"a } b"}'` deliberately DROPPED (traced: produces invalid JSON, indistinguishable). (2) NEW `ARTICLE_RESPONSE_SCHEMA` (OpenAPI-style, `as const`) wired into generationConfig alongside responseMimeType application/json (`{ responseMimeType, responseSchema }`) — gemini-2.5-flash structured output must then emit well-formed JSON, preventing the corruption at the source (escapeInnerQuotes stays as fallback). (3) `validate` now EXPORTED (testability; also `export` proof from inside function). (4) Enriched fail reason: `json_parse_fail_after_object_extract:${len}:${sample(120)}` — user debug shows WHY parse failed; safe because corrective lookup is `CORRECTIVE_PROMPTS[reason] ? reason : reason.split(':')[0]`. (5) responseMimeType-rejection retry regex widened `|schema` (some Gemini builds reject the mime+schema+googleSearch combo). Test suite: `escapeInnerQuotes` unit tests (`the "AI", model is great` → `the \"AI\", model is great`; `the "term": is used` → escaped; `["hi", "bye"]` untouched) + `describe("validate")` (ok fixture passes; unrecoverable blob → enriched reason). First vitest run FAILED (1/105) — pretty-printed fixture (`"AI chips boom",\n  "seoTitle"`) hit the space-only skip → whitespace fix applied → **105/105 green (10 files)**, tsc clean. NOTE: the whitespace-skip + look-ahead applies to ALL THREE skip loops (the `j` loop + both `k` loops — the `,` and `:` branches share identical k-loop text). No DB/migration/edge-fn changes. Carry-over user items unchanged: delete Vercel dashboard cron `fetch-trending-keywords` 03:45 UTC (manual, dashboard → Settings → Cron Jobs; route + edge fn already kill-switched), real AdSense publisher ID (placeholder pub-3867128949973557 + enable_auto_ads=false).
