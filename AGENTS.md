...
- **TURNSTILE KEYS IN VERCEL PRODUCTION + PREVIEW ENV** — Both `NEXT_PUBLIC_TURNSTILE_SITE_KEY` and `TURNSTILE_SECRET_KEY` confirmed live in Vercel production env (CLI-added). Preview env vars added via REST API. Fresh deploy triggered (docs commit) so `NEXT_PUBLIC_TURNSTILE_SITE_KEY` is baked into the production client bundle. Cloudflare account: Account ID `b33b362097911fe86e6e90cf57597b64`, Zone ID `34dad511ce1f4d93da5ffa5cbd13f233`. WAF: Bot Block ruleset `aafddb14081047cc86739282c9e06fe2` (blocks empty UA + scrapers on non-API), Auth Rate Limit `3cf5137ecea643d4a1f45e200e90afd3` (10 req/10s on login+signup, block, mitigation 10s). Zone settings verified: security_level=medium, always_use_https=on, min_tls=1.2. HIBP: user says paid (~$3.50/mo) — skipped for now, code handles missing key gracefully.
