...
- **Current Phase:** **Analytics Center + Report Center full rebuild (commit pending; migration 046 NOT yet applied to live DB)** — Analytics page: Overview now splits **Site Views vs Post Views** (dual-line 30d chart + separate KPIs), **Sessions = distinct session_id** (new column, tracked per-tab), Real-Time shows Active Visitors + Sessions Today, Audience tab now has real data (device/browser/os columns — the old tab queried NONEXISTENT columns and was always empty), Newsletter tab fixed (was querying `subscribers.created_at` which doesn't exist — uses `subscribed_at` + real `newsletter_sends`), Exports tab real CSV w/ session/device columns + Daily Views CSV. **Revenue tab was silently $0.00** (queried `ad_revenue.amount`/`created_at`; real cols are `revenue`/`date`, `affiliate_sales.converted_at`) — fixed + light theme. **Report Center rebuilt**: new shared `src/lib/reports.ts` (data fetch + MD/CSV builders for all 6 reports from REAL data — no more hardcoded SEO estimates), API `/api/admin/reports` (generate + schedule CRUD, admin/editor guarded, service-role writes), cron `/api/cron/reports` (CRON_SECRET, emails via sendBrandedEmail), page with real generate MD/CSV/Export All + **Schedule Reports** UI (type/frequency/format/email → `report_schedules` table). Quick stats fixed (`profiles` not `user_profiles`, + subscribers). **Migration 046 `analytics_v2_reports`** (NOT applied to live yet): +session_id/device/browser/os on analytics_events + index, `report_schedules` table (RLS + realtime). Trackers (`page-view-tracker`, `view-tracker`) now send session_id/device/browser/os via new `src/lib/view-tracking.ts`; `/api/increment-views` stores them.
