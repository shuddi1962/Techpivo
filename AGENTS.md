...
- **REPORT ROUND COMMITTED (64800d6, PUSHED) — Next 14 params crash fixed on quiz + post detail (500s + dead voting), forum meta NOT NULL fixed, duplicate community nav removed, polls/ActivePolls 15s + LIVE, advertise landing blue rebrand with geo-currency pricing (public + account + admin)** — User: "quize page content shows 500", "Discover feed clicking a post shows 500 and voting won't save", "poll page images not showing/not realtime", "update admin and accounts plus public" for currency. **ROOT CAUSE of quiz 500 + Discover post 500**: project is **Next.js 14.2.35** (confirmed package.json + dev.log) — `params` is a PLAIN OBJECT, not a Promise; two client pages used Next-15 `params.then()` syntax → crash on mount → error boundary. Fixed in `src/app/community/quiz/[id]/page.tsx` (fetch + attempt blocks) and `src/app/community/forum/[category]/[id]/page.tsx`. **Voting dead second bug**: post detail sent `vote_type: 1|-1` (numeric) but API requires `'up'|'down'` → 400; `handleVote(type, id, voteType: 'up'|'down')` now sends strings + updates counts only on res.ok + reply votes wired. **Forum post creation 500**: `forum_posts.meta` is `jsonb NOT NULL DEFAULT '{}'` — API sent `meta: null` → NOT NULL violation; fixed `meta: Object.keys(meta).length ? meta : {}` in `/api/community/posts` (line 271). **Duplicate nav removed**: `src/components/community/community-header.tsx` DELETED (was rendered alongside CommandCenter on community + answers layouts) — both layouts now render CommandCenter only. Scanned src: no other `params.then`; remaining `await params` are server components/API routes (safe). **Polls**: images verified valid Pexels URLs (200 image/jpeg) + realtime healthy (increment_poll_votes updates polls.total_votes + poll_options.vote_count → UPDATE broadcasts); ActivePolls cards now show images (h-36 + gradient overlay) + LIVE badge + "Updated {time}", interval 30s→15s; `/community/polls` page same (15s + LIVE badge + last-refresh time). **Advertise landing rebuilt** (`advertise-landing.tsx`): blue brand (#2563EB/#1D4ED8, blue gradient CTA), LIVE stats band REMOVED (no /api/ads/stats polling), "Placements, Live" section removed → new "Choose Your Ad Space" pricing grid with **geo-detected currency** (`useFx` displayCurrency + FX_POPULAR switcher + "we detected your location" copy), Ad Formats section now shows REAL image/video examples (Pexels 3184291/1181671/159306/1181244 with overlay + sizes + VIDEO badge), chat mock price now converted, FAQ currency answer updated (auto-detect + 10 currencies). **Account ads**: `/account/ads/new` default currency now geo-detected via `getGeoOnce` + `COUNTRY_TO_CURRENCY` (respects FX_PREF_KEY); `/account/ads` Total Spend KPI now converts each campaign's spend from its own currency to display currency (`useFx().convert`) — was summing raw mixed-currency amounts as ₦. **Admin ads**: SpacesTab + ManageTab min bids now render in visitor currency via `useFx` (`fx.format(p.min_bid_cpm,'NGN')`); KPIs already had FxApprox. Verified BEFORE push: tsc EXIT=0, lint 0 errors (pre-existing img/exhaustive-deps warnings only), vitest 53/53, `next build` EXIT=0, dev smoke 8/8 200 (/advertise, quiz list + [id], forum [category]/[id], polls, account/ads/new, account/ads, admin/ads). NOTE: getGeoOnce bails on api-key-less setups → falls back NGN. Commit 64800d6 (12 files, +230/−245, deleted community-header.tsx). GOTCHA: Current Status top bullet drifts from system-prompt copy — always anchor edits on the ACTUAL on-disk line (4c178d5 is the disk top bullet after 0dd39d3 was committed to session-history only).
