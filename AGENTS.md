...
- **PAGES CENTER POLISH COMMITTED (7f49234, PUSHED) — slug auto-gen from title, revalidatePath on all mutations, design_settings wired, raw HTML blocks in markdown** — User: "continue". 5 uncommitted page improvements finalized: (1) Create Page dialog simplified — slug auto-generated from title via `slugify()` + collision loop (removed separate `newSlug` input field + manual validation); (2) `/api/admin/pages` route gains `revalidatePath("/")` + `revalidatePath(\`/\${slug}\`)` on reset/delete/toggle/save so public pages update instantly without ISR wait; (3) `page-intro.tsx` + `page-shell.tsx` wire `design_settings` JSONB (hero_bg, text_color, content_width, hero_alignment, hero_height) — hero band applies inline styles from DB; (4) `page-shell.tsx` switches `<Image>` → `<img>` to bypass Vercel Hobby plan 402 optimizer (matches ad-components fix 7afca37); (5) `markdown.ts` gains `{html}...{/html}` raw block syntax for embedding custom HTML in page markdown. Migration 082 (design_settings + placement) already applied live. Verified BEFORE push: tsc EXIT=0 (NODE_OPTIONS=--max-old-space-size=4096), next lint 0 errors (pre-existing img warning only). Carry-over user items unchanged: delete Vercel dashboard cron `fetch-trending-keywords` 03:45 UTC, real AdSense publisher ID (placeholder pub-3867128949973557 + enable_auto_ads=false).
