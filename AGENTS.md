...
- **PAGESHELL FULL-WIDTH HERO + BODY (b8c06c5, PUSHED) — hero banner now stretches edge-to-edge (object-cover), body content full-width (card wrapper removed)** — User: "now the pages the hero banner is not stretch and full with make it that way including the body of the pages". `src/components/pages/page-shell.tsx`: hero image `object-contain` → `object-cover` (stretches to fill banner); body content outer div changed to `w-full py-10` (removed `px-4 md:px-12 lg:px-16`); non-fullWidth path still has article-level padding for readability; card wrapper (`rounded-2xl border border-borderSoft bg-surface shadow-sm`) removed. Verified: tsc clean. Prior items unchanged: page editor hero image search (54e912a) active, global full-width revert (cfce912) still in place.
