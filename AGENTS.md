...
- **OG:IMAGE PROTOCOL FIX + FAVICON VERIFIED (PUSHED 49d45cb) — article pages now emit absolute `https://` `og:image` URLs with a site-default fallback; favicon metadata/assets confirmed complete (8783d0a)** — User: "open graph og:image tag protocol also for the Fix favicon not showing in Vercel project dashboard use our sites own in the browser". **(1) OG fix (`src/app/[slug]/page.tsx`, commit 49d45cb, +18/−2)**: NEW inline `toAbsoluteImageUrl(url?)` helper (passes `http(s)://` through, `//`→`https:`, `/`→`SITE_URL+path`, else `SITE_URL+"/"+u`; trims); `generateMetadata` now computes `ogImage = toAbsoluteImageUrl(post.og_image) || toAbsoluteImageUrl(post.featured_image) || \`${SITE_URL}/opengraph-image\`` — ROOT CAUSE of missing default: `openGraph.images: []` when no image OVERRIDES the file-convention default (`src/app/opengraph-image.tsx`, commit 0190911). New openGraph: `url: canonical`, `siteName: SITE_NAME`, `type: "article"`, `images: [{ url: ogImage, width: 1200, height: 630, alt: post.title }]`; `twitter.images: [ogImage]` (was raw DB value or `undefined`). **(2) Site-wide audit — ONLY `[slug]` emitted raw DB image URLs in metadata**: all other dynamic pages verified (answers/[slug], community/forum/[category], u/[username] — openGraph has NO images so they inherit the root default; author/[username], category/[slug] + [subcategory], tag/[tag], tools/*, contact, preview/[slug] — no openGraph images set; grep confirms only `[slug]/page.tsx` uses `og_image|featured_image` in metadata). Page-body `<img>`/`backgroundImage` featured_image uses elsewhere are render-only, not OG tags. **(3) Favicon already complete (8783d0a)**: `layout.tsx` icons metadata (favicon.ico, icon.svg, icon-192.png, icon.png 512, shortcut, apple) + all assets verified live (public/favicon.ico, favicon.svg, icon.png, icon-192.png, src/app/favicon.ico, icon.svg, opengraph-image.tsx). If Vercel dashboard still shows an old icon after this deploy it is Vercel's server-side avatar cache refreshing on their schedule — not a code issue. Verified BEFORE push: tsc EXIT=0 (NODE_OPTIONS=--max-old-space-size=4096), next lint `src/app/[slug]/page.tsx` 0 errors. Git: HEAD `49d45cb`, working tree clean. Carry-over user items unchanged: real AdSense publisher ID (placeholder pub-3867128949973557 + enable_auto_ads=false).
