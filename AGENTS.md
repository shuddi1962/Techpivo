...
- **P9+P10 committed (0c26ebe) — notifications triggers + Moderation Center LIVE, migration 061 APPLIED** — P9: `061_community_notification_triggers.sql` applied live (verified): `notify_community()` SECURITY DEFINER helper (checks `user_profiles.notification_preferences` jsonb keys forum_replies/new_followers, inserts into user_notifications), `trg_notify_forum_reply` (AFTER INSERT forum_replies → post author, skips self, link `/answers/{slug}?focus=` for answer type else `/community/forum/{cat}/{id}`), `trg_notify_follow` (AFTER INSERT user_follows), perf indexes `idx_user_notifications_user_read`, `idx_forum_replies_post_type`, `idx_forum_posts_type_status`, `idx_forum_posts_author_created`, `idx_user_follows_follower`. Notifications API extended: GET `?count=true` → `{unread}` + POST `{action: read|read_all, id}`; answers POST route drops its duplicate service-role notification (trigger owns it now; PATCH accept keeps its own). P10: **`POST /api/community/report`** (auth required, `RATE_LIMITS.report` 10/h via `checkRateLimit().allowed`, target_type whitelist forum_post/forum_reply/comment/user, reason whitelist, details cap 1000, dedupe open reports per reporter 409, insert via RLS client); **`GET/POST /api/admin/moderation`** (`requireAdminRole(['admin','editor'], request)` + `createServiceClient` — content_reports has NO admin-select RLS and NO FK on reporter_id/author_id (all dropped in 049) so joins are impossible → batch profile enrichment; previews: post title/content, reply content + parent post title (valid FK `forum_replies_post_id_fkey`), comment, user profile; GET returns stats open/total/pending_today + 200 reports; POST actions: dismiss (status dismissed + audit_logs best-effort insert — real cols user_id/action/entity_type/entity_id uuid/details jsonb), remove (forum_post → is_locked=true + meta.hidden, forum_reply → service DELETE, comment → is_approved=false, user → 400 use warn), warn (user_notifications insert type moderation + resolved)); **`/admin/moderation`** white Moderation Center: KPIs (Open/Today/Total), tabs Open/All/Resolved, per-report card (target icon, status pill, reason pill, timeAgo, target preview box, reporter, View links to post/parent, details quote), actions Remove (confirm)/Warn user (confirm)/Dismiss, realtime unique channel on content_reports + 30s poll + focus + LIVE badge; sidebar "Moderation" (ShieldAlert). Verified: tsc clean, lint clean (fixed &ldquo; escapes), dev smoke 3/3 — /admin/moderation 200, report POST unauth 401, moderation GET unauth 401 (both guards correct).
