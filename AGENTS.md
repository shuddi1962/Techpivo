...
- **HOMEPAGE OG IMAGE = REAL SITE SCREENSHOT (PUSHED e578e7b) — root openGraph/twitter metadata now point to `/og-home.png` (1200×630 real homepage capture via headless Chrome, 329 KB) so social shares of techpivo.com show the actual homepage instead of the branded gradient card; file-convention `opengraph-image.tsx` still generates `/opengraph-image` but explicit `openGraph.images` overrides it on the root** — Continues the OG round (49d45cb protocol fix + default fallback). Capture: `& "C:\Program Files\Google\Chrome\Application\chrome.exe" --headless=new --disable-gpu --user-data-dir="$env:TMPC" --screenshot=public/og-home.png --window-size=1200,630 --hide-scrollbars --virtual-time-budget=9000 https://techpivo.com/` (needs a unique temp user-data-dir or headless silently produces no file; first attempt without it FAILED). Verified: PNG 1200×630 (System.Drawing). `layout.tsx` openGraph.images `[{url: SITE_URL/og-home.png, 1200, 630, alt: SITE_NAME — SITE_TAGLINE}]` + twitter.images `[SITE_URL/og-home.png]`. Verified BEFORE push: tsc EXIT=0, next lint `src/app/layout.tsx` 0 errors. Working tree clean after push. **FAVICON QUESTION ANSWERED (no code fix exists)**: user asked "why is vercel favicon showing instead of our own" — live `techpivo.com` serves OUR favicons byte-identical to repo (SHA256 verified: /favicon.ico 2913 B `90E39F86...`, /icon.png 6522 B `372426A7...`, /icon-192.png 2236 B `687284E5...`, /icon.svg 464 B navy+amber); the Vercel triangle sighting = (a) Vercel DASHBOARD project avatar (Vercel-side setting: Project → Settings → General → Project Avatar → upload public/icon.png — manual, no code path), (b) browser tab favicon cache (hard refresh/incognito), or (c) a Vercel-branded 404/error page. Repo has zero Vercel assets (only vercel.json + .env.vercel). Carry-over user items unchanged: real AdSense publisher ID (placeholder pub-3867128949973557 + enable_auto_ads=false).
