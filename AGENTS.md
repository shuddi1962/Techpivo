...
- **Current Phase:** **Ad Marketplace V3 — Professional landing page + Admin Analytics (commit pending after landing rebuild)** — /advertise is now a static marketing landing (hero, trust bar, audience cards, ad-format mockups, 4-step how-it-works, features, tracking showcase, FAQ, CTA) with ZERO placements/form — campaigns created only in the user account (My Ads → /account/ads/new). Admin /admin/ads gained an Analytics tab: 14/30-day toggle, 5 KPIs, ComposedChart (impressions bars + clicks line), revenue AreaChart, top-campaign + placement performance from API `section=analytics`, realtime on ad_campaign_daily_stats. Auction model live (migrations 044 + 045 applied live): advertisers set daily budget + bid (CPM/CPC) against placement `min_bid_cpm`/`min_bid_cpc` floors, 10 currencies with live fx_rates in site_settings, campaign goals + CTA types + target audience chips, video ads on video-capable placements, AI creative generator (Gemini), advertiser account area /account/ads + /account/ads/[id] (KPIs + 14-day chart) + /account/ads/new.
...
| 2026-08-10 | **Landing + Admin Analytics Rebuild (commit pending)** | Per user request: **/advertise is now a pure professional marketing landing page** — NO "Available Ad Spaces" grid, NO "Build Your Campaign" form, no pricing/currency widgets. Static sections: hero (audience value prop + Start Your Campaign → `/account/ads/new` + ads@techpivo.com), trust bar (20k readers/11 categories/10+ placements/10 currencies/24h approval), audience cards (developers/IT pros/gadget buyers/learners), ad-format mockups (leaderboard 728x90, rectangle 300x250, in-content 336x280, VIDEO), 4-step How It Works (all "in your account"), 6 "Why Techpivo" feature cards, tracking showcase with sample live-analytics mock chart, 8-item FAQ, gradient CTA → account. Users commit ONLY in their account (My Ads). **Admin Ads page: new Analytics tab** (`BarChart3`, 2nd tab): API GET `section=analytics` (aggregates `ad_campaign_daily_stats` + `ad_revenue` into 30-day daily series w/ zero-fill, top 8 campaigns by impressions w/ spend, per-placement stats); page fetches it in loadAll + realtime on `ad_campaign_daily_stats`; tab has 14/30-day toggle, 5 KPIs (impressions/clicks/CTR/spend/ad revenue), recharts ComposedChart (impressions bars + clicks line, dual Y axes), revenue AreaChart with gradient, top-campaigns progress bars w/ status + spend, placement performance bars. tsc + lint clean. |
