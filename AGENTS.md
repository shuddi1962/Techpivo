...
- **Current Phase:** **Ad Marketplace V2 (code complete, migration 044 pending manual apply)** — video ads, weekly/monthly billing, multi-currency (10 currencies, live fx_rates in site_settings), campaign goals + CTA types + target audience (country/device/interest chips), AI creative generator (Gemini), public /advertise page in nav, realtime placements on public page. NOTE: migration 044 must be applied to live DB via SQL Editor before V2 UI works (columns price_per_week/price_per_month/supports_video on ad_placements; goal/cta_type/target_audience/currency/fx_rate/billing_frequency/media_type/video_url/poster_url on ad_campaigns; fx_rates setting). Fixed constraint bug: ad_type CHECK must keep legacy types (native/infeed) — live rows use them.
