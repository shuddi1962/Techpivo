...
- **Current Phase:** SEO Center fully working + **One-Click Auto-Fix Engine live** — fix/resolve issues directly from Dashboard, Issues, Technical SEO, Image SEO tabs (fix API now authenticated: admin/editor only, service-role writes, `resolve_type` action; per-issue Fix/Resolve buttons in audit details); all 237 posts now have content images (235 auto-fixed via `/api/admin/seo/fix`); re-audited scores 82-92. Production stable.
...
| 2026-08-09 | Newsletter Real Sending Live (commit 61ea195) | Completed the uncommitted branded-email work. New `src/lib/email.ts`: shared Techpivo branded HTML template (`brandedEmail()`, preheader/CTA/unsubscribe/footer) + `sendBrandedEmail()` via Resend from `newsletter@newsletter.techpivo.com` (domain verified; `replyTo` camelCase — resend v6 SDK rejects `reply_to`). `src/lib/newsletter.ts` rewritten: `sendNewsletterForPost` → branded template + category-targeted subscribers; new `sendNewsletterCampaign` (batches of 50 + 2s delay, returns `{total, delivered, failed}`, 500 if `RESEND_API_KEY` missing). **Admin Newsletter send-campaign now REALLY delivers** (`/admin/newsletter/api` POST: fetches campaign + active subscriber emails, 400 if none, sends via campaign's `html_content` (new DB column, migration 041 applied) or fallback body, updates `sent_count` = delivered, returns recipient/failed counts). Admin page: campaign composer has a message field (plain text or HTML), Send has confirm dialog + result alert. Signup route sends branded welcome email (fire-and-forget). Contact route uses branded template + `replyTo` + `CONTACT_TO_EMAIL` env (default hello@techpivo.com). Fixed 1 tsc error (`reply_to`→`replyTo`); tsc + next lint clean; verified `html_content` column exists in DB. Committed 61ea195 (7 files). NOTE: ingest → `sendNewsletterForPost` unchanged signature; only 2 active subscribers in DB currently — real delivery test needs ≥1 active subscriber + RESEND_API_KEY in prod env. |
